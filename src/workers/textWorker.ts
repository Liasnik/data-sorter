...
self.onmessage = (event: MessageEvent<Actions>) => {
  const post = (data: Result) => (self as unknown as { postMessage: (r: Result) => void }).postMessage(data)
  ...
  try {
    if (message.type === 'dedup') {
      ...
      post({ id: message.id, ok: true, with: joinLines(uniqueOnly), without: joinLines(duplicatesOnly) })
      return
    }
